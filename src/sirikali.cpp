...
static void _connect_to_ssh_server( QWidget * parent,const favorites::entry& entry )
{
	...
	if( s.cmdStatus() == engines::engine::status::sshfsNotFound ){
		...
		auto _mount = [ & ]( favorites::volumeList& q,
			...
			if( s ){
				...
				if( s.cmdStatus() == engines::engine::status::success ){
					...
					if( s.backendDoesNotAutoRefresh() ){
