...
{
    return LXQt::Wallet::Task::run<result>([this]()->result
    {
	db In(m_data);
...
LXQt::Wallet::Task::future<LXQt::Wallet::windows_dpapi::result> &LXQt::Wallet::windows_dpapi::decrypt()
{
    return LXQt::Wallet::Task::run<result>([this]()->result
    {
	db In(m_data);
...
{
    this->decrypt().then([ this ](const result &m)
    {
        if (m.success)
        {
