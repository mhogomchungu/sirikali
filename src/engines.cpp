...
engines::engine::cmdStatus::cmdStatus( engines::engine::status s,
				       ...
				       const QStringList& e ) :
	m_status( s ),
	m_backendExtensionNames( e ),
	m_engine( n )
{
}

engines::engine::cmdStatus::cmdStatus( int s,
				       const engines::engine& n,
				       const QString& e ) :
	m_exitCode( s ),m_engine( n )
{
	this->message( e ) ;
}
...
QString engines::engine::cmdStatus::toString() const
{
	...
	{
		auto a = m_engine.engine().name() ;
		auto b = m_engine.engine().minimumVersion() ;
